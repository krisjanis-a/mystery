...
class SettingsController extends Controller
{
    ...
    public function requestLastfmAuthorization ()
    {
        ...
        $authEndpoint = "http://www.last.fm/api/auth/?api_key=".env('LASTFM_API_KEY')."&cb=http://localhost:8000/settings/create-lastfm-session";
    ...
    public function requestSpotifyAuthorization (Request $request)
    {
        ...
        return redirect('https://accounts.spotify.com/authorize?client_id='.env('SPOTIFY_CLIENT_ID').'&response_type=code&redirect_uri='.$redirect_uri);
    }
    ...
    public function connectSpotifyAccount(Request $request)
    {
        ...
        */
        ...
        // Add response data to database
        DB::update('update users set spotify_access_token = ?, spotify_refresh_token = ?, scope = ? where id = ?', [$access_token, $refresh_token, 'playlist-read-private', $user_id]);
